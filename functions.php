...
function add_site_scripts()
{
    ...
    wp_register_script('jquery-core', get_template_directory_uri() . '/js/jquery.min.js', false, false, true);
...
function query_filter()
{
    if ((isset($_GET['post_type']) || isset($_GET['sort']) || isset($_GET['posts_per_page']))
        ...
        && (strpos($_SERVER['QUERY_STRING'] ?? '', 'filter_') === false)
    ) {
        global $wp_query;
...
{
    $args_raw = $jx->data['query'] ?? '';
    $args_un = @unserialize(is_string($args_raw) ? stripslashes($args_raw) : '');
    $args = is_array($args_un) ? $args_un : [];
    $args['post_status'] = 'publish';
    $args['paged'] = (int)($jx->data['page'] ?? 0) + 1;
    if (isset($args['ID']) || !empty($jx->data['archive'])) {
        if (isset($args['ID'])) {
    ...
    ob_start();
    $tpl = locate_template('template-parts/' . ($jx->data['part'] ?? '') . '.php');
    if ($tpl) {
        require $tpl;
    }
    return ob_get_clean();
...
function ajaxs_filter($jx)
{
    ...
    foreach ($jx->data as $key => $value) {
        if ($value !== '') {
            if ($key === 's') {
                ...
                if ((is_array($value) ? trim(implode('', $value)) !== '' : trim((string)$value) !== '')) {
                    $args['tax_query'][] = [
                        'taxonomy' => $key,
                        'field' => $field,
                        'operator' => $operator,
    ...
    ob_start();
    $tpl = locate_template('template-parts/' . ($jx->data['post_type'] ?? '') . '.php');
    if ($tpl) {
        include $tpl;
    }
    return ob_get_clean();
...
function fix_acf_field_keys()
{
    // Проверяем, был ли запрошен запуск
    if (!isset($_GET['update_acf_keys']) || $_GET['update_acf_keys'] !== 'true') {
        return;
    }

    // Проверяем права пользователя
    if (!current_user_can('manage_options')) {
        wp_die('У вас недостаточно прав для выполнения этой операции');
    }

    // Получаем все группы полей ACF
    $field_groups = acf_get_field_groups();

    // Собираем все поля ACF в массив [ 'имя_поля' => 'field_ключ' ]
    $acf_fields = [];
    ...
    if (empty($acf_fields)) {
        wp_redirect(admin_url('tools.php?page=config&message=no_acf_fields'));
        exit;
    }

    // Находим все записи, у которых есть мета-поля ACF
    $args = [
        'post_type' => 'any',
        'posts_per_page' => -1,
    ...
    foreach ($query->posts as $post_id) {
        ...
        foreach ($meta_values as $meta_key => $meta_value) {
            ...
            if (array_key_exists($meta_key, $acf_fields)) {
                $field_key = $acf_fields[$meta_key];
                $hidden_meta_key = '_' . $meta_key;

                // Проверяем и обновляем при необходимости
                if (get_post_meta($post_id, $hidden_meta_key, true) !== $field_key) {
                    update_post_meta($post_id, $hidden_meta_key, $field_key);
                    $updated = true;
                }
            }
        }
    ...
    // Обновление ключей ACF для опций
    foreach ($acf_fields as $field_name => $field_key) {
        $hidden_option_name = '_options_' . $field_name;
        if (get_option($hidden_option_name) !== $field_key) {
            update_option($hidden_option_name, $field_key);
            $options_updated++;
        }
    }

    // Перенаправляем с сообщением о результате
    wp_redirect(admin_url('tools.php?page=config&message=acf_keys_updated&updated_posts=' . $updated_posts . '&updated_options=' . $options_updated));
    exit;
}

// Добавляем обработчик для admin_init
add_action('admin_init', 'fix_acf_field_keys');

// Функция для вывода сообщения об успехе/ошибке
function show_acf_update_notice()
{
    ...
    $messages = [
        'no_acf_fields' => [
            'class' => 'error',
            'text' => 'Не найдено ни одного поля ACF.'
        ],
        'acf_keys_updated' => [
            'class' => 'success',
            'text' => 'Ключи ACF успешно обновлены. Обновлено записей: ' . (isset($_GET['updated_posts']) ? intval($_GET['updated_posts']) : 0) . '. Обновлено опций: ' . (isset($_GET['updated_options']) ? intval($_GET['updated_options']) : 0)
        ]
    ];
...
add_action('admin_notices', 'show_acf_update_notice');

function wtwSetUrl(string $path = ''): string
{
...
add_action('wp_footer', 'wtw_ajax_load_part_inline', 999);

function wtw_ajax_load_part_inline() {
    ?>
    <script>
    jQuery(function($) {
        $(document).on('click', '[data-ajax-load]', function(e) {
            ...
            var template = $btn.data('ajax-load');
            ...
            // если data-part не указан → ничего не делаем
            if (!part || typeof part !== 'string' || part.trim() === '') {
                return;
            }

            var target = '.part-' + part.trim();

            ajaxs('wtw_load_chunk', {
                post_id  : $btn.data('post-id'),
                target   : target,
                template : template,
                method   : $btn.data('method') || 'replace'
            });
        });
    });
    </script>
...
function ajaxs_wtw_load_chunk($jx) {
    $post_id  = (int) ($jx->post_id  ?? 0);
    $target   = $jx->target   ?? '';
    $method   = $jx->method   ?? 'replace';
    $template = $jx->template ?? '';

    if (!$post_id || !get_post($post_id)) {
        $jx->error('Неверный ID записи');
        return;
    }

    if (!$template) {
        $jx->error('Не указан шаблон');
        return;
    }

    if (!$target) {
        $jx->error('Не указан целевой селектор');
        return;
    }
    ...
    if ($query->have_posts()) {
        ...
        $jx->html($target, $html, $method);
        $jx->success();
    } else {
        $jx->error('Запись не найдена');
    }
}